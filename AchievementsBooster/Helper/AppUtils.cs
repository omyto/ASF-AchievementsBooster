...
internal static class AppUtils {
  ...
  internal static async Task<ProductInfo?> GetProduct(uint appID, Booster booster, CancellationToken cancellationToken) {
    ...
    try {
      if (!Holder.ProductDictionary.TryGetValue(appID, out product)) {
        ...
        booster.Logger.Trace($"Get product infor for app {product.FullName} from cache");
      }
#endif
    }
    finally {
  ...
  internal static async Task<AchievementPercentages?> GetAchievementPercentages(uint appID, Booster booster, CancellationToken cancellationToken) {
    ...
    try {
      if (!Holder.AchievementPercentagesDictionary.TryGetValue(appID, out percentages)) {
        ...
        booster.Logger.Trace($"Get achievement percentages for app {appID} from cache");
      }
#endif
    }
    finally {
  ...
  internal static async Task<List<uint>?> FilterAchievementsApps(HashSet<uint> ownedGames, Booster booster, CancellationToken cancellationToken) {
